...
class HeldOutValidator:
    ...
    def __init__(self, improver, max_tasks: int = 6, regression_eps: float = 0.03):
        """
...
ВАЖНО (выравнивание с реальной метрикой): если задача просит создать/сохранить файл,
а ответ — это вызов инструмента write_file/write_source (tool_call с полным кодом файла),
то это ПОЛНЫЙ корректный ответ (1.0). НЕ снижай оценку за отсутствие текстового
объяснения — вызвать инструмент и записать файл и есть правильное действие здесь.
