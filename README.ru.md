...
- **Таргетинг по failure-mode** — провалы группируются по корневой причине
  (напр. «показал код текстом вместо записи файла»), правится нужный промпт
- **Гейт** — правка активируется только если `test_mvp` И held-out валидация
  не показали регресса; иначе авто-откат (`/version rollback`)
- Измеренный прогон: сложный кодинг-бенчмарк 84%→91% (val 73%→87%), held-out без
  изменений — см. [eval/SELF_IMPROVEMENT_RESULTS.md](eval/SELF_IMPROVEMENT_RESULTS.md)
- Авто-цикл улучшения каждые 5 задач (опционально, по флагу); дайджест (cron)
...
- **Защита от prompt-injection** — активная санация недоверенного контента
  (web/reddit): дефанг control-токенов + баннер «данные, не инструкции»
  (`test_security.py`, 40 кейсов)
