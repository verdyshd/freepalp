...
- **持久记忆**——HOT/WARM/COLD 分层 + 可在界面里浏览的真实向量图，外加对整个
  会话历史的 **FTS5 搜索**。
- **自我改进**——按根本原因聚类自身失败（例如"以文本展示代码而非写入文件"），改写相关提示词，仅当确定性测试套件（`test_mvp`）和留出集验证均无回归时才上线，否则自动回滚。实测一次：困难编码基准 84%→91%（val 73%→87%），留出集不变——见 [eval/SELF_IMPROVEMENT_RESULTS.md](eval/SELF_IMPROVEMENT_RESULTS.md)。
